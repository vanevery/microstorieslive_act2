...
function buildCharacter( headTex, bodyTex, armULTex, armDLTex, armURTex, armDRTex,legULTex, legDLTex, legURTex, legDRTex ){
	...
	//JOINTS
		...
		for(var i=0; i<jointsPos.length; i++){
			...
			var tV = new THREE.Vector3( jointsPos[i].x, -1*jointsPos[i].z, jointsPos[i].y );
...
function update(){
	...
	if(characterBuilt && elmoBone) {
		elmoBone[1].position.copy( elmoAdjustJoints[0] );	//head
		elmoBone[19].position.copy( elmoAdjustJoints[1] );	//root
		elmoBone[12].position.copy( elmoAdjustJoints[2] );	//palmL

		elmoBone[11].position.copy( elmoAdjustJoints[3] );	//armL

		elmoBone[17].position.copy( elmoAdjustJoints[4] );	//palmR
		elmoBone[16].position.copy( elmoAdjustJoints[5] );	//armR

		elmoBone[23].position.copy( elmoAdjustJoints[6] );	//feetL
		// elmoBone[22].position.copy( elmoAdjustJoints[7] );	//kneeL

		elmoBone[28].position.copy( elmoAdjustJoints[8] );	//feetR
		// elmoBone[27].position.copy( elmoAdjustJoints[9] );	//kneeR
	}
