...
function initThreeJS() {
	...
	// ELMO
		// elmoTex = THREE.ImageUtils.loadTexture('images/slipper.png');
		elmoMat = new THREE.MeshLambertMaterial( {color: 0xff0000} );
		// loadModelElmo("models/elmo5.js",elmoMat);
		...
		// 	var tmpV = THREE.Vector3();
		// 	tmpV = joints[i].position;

		// 	elmoAdjustPosition.push(tmpV);
		// }
...
function loadModelElmo (model, meshMat) {
	...
	loader.load(model, function(geometry, material){
		...
		// elmoMattt.shadding = THREE.FlatShading;
		// elmoMattt.color = new THREE.Color( 0xff0000 );
		elmoGeo = geometry;

		elmoRig = new THREE.SkinnedMesh(elmoGeo, elmoMattt);

		// elmoRig.position.set(0,0,75);
		// elmoRig.scale.set(2,2,2);


		scene.add(elmoRig);

		// elmoSuit.add(elmoRig);
		// elmoSuit.scale.set(5,5,5);
		// scene.add(elmoSuit);
...
function loadModelElmo2 (model, meshMat) {
	...
	loader.load(model, function(geometry, material){
		...
		*/
		...
		// elmoRig = new THREE.SkinnedMesh(elmoGeo, eMat);
		elmoRig = new THREE.Mesh(elmoGeo, eMat);

		// elmoRig.position.set(5,5,5);
		// elmoRig.scale.set(5,5,5);
...
function update(){
	...
	if(audienceLines.length>0){
		for(var i=0; i<audienceLines.length; i++){
			...
			// worldVector.setFromMatrixPosition( audienceLines[audienceLines.length-1].matrixWorld );
			...
			// console.log( audienceLines[0].position );
		}
	}
	...
	if(characterBuilt && elmoBone) {
		...
		// OLD_v2
		// elmoBone[0].position.copy( joints[5].position.clone().divideScalar(10) );
		// elmoBone[1].position.copy( joints[4].position.clone().divideScalar(10) );

		// elmoBone[2].position.copy( joints[3].position.clone().divideScalar(10) );
		// elmoBone[3].position.copy( joints[2].position.clone().divideScalar(10) );

		// elmoBone[4].position.copy( joints[1].position.clone().divideScalar(10) );
		// elmoBone[5].position.copy( joints[0].position.clone().divideScalar(10) );

		// elmoBone[6].position.copy( joints[9].position.clone().divideScalar(10) );
		// elmoBone[7].position.copy( joints[8].position.clone().divideScalar(10) );

		// elmoBone[8].position.copy( joints[7].position.clone().divideScalar(10) );
		// elmoBone[9].position.copy( joints[6].position.clone().divideScalar(10) );
