...
//THREE.JS
	...
	var jointsCat = [];
	var jointsPosCat = [
		new THREE.Vector3(0, 4, 0.1),		//neck
		new THREE.Vector3(0, -4, 0),	//body
...
function init() {
	...
	var catTexture = THREE.ImageUtils.loadTexture('images/stickCatBodyUV.png');
	catMaterial = new THREE.MeshLambertMaterial( {map: catTexture} );

	loadModelCat("models/stickCat/headC.js",
				 "models/stickCat/bodyD.js",
				 ...
				 "models/stickCat/leg_L_UC.js", catMaterial);
...
function loadModelCat (model0, model1, model2, model3, model4, model5, model6, model7, model8, model9, catM) {
	...
	loader.load(model1, function(geometry1){
		...
		transZ(geometry1, 4);
		// transZ(geometry1, 4);
		bodyCat = new THREE.Mesh(geometry1, catM);
		bodyCat.position.copy(jointsPosCat[1]);
		scene.add(bodyCat);
	}, "js");
